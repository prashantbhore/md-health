...
class VendorProductController extends Controller
{
    ...
    public function add_vendor_product(Request $request){
        ...
        if ( $request->hasFile( 'vendor_product_image_path' ) && $request->file( 'vendor_product_image_path' )->isValid() ) {
            $image = $request->file( 'vendor_product_image_path' );
            $image_name = 'vendor_product_image_path';
            $responseData = $this->apiService->getData( $token, $apiUrl1, $plainArray, $method, $image, $image_name );
        } else {
            $responseData = $this->apiService->getData( $token, $apiUrl1, $plainArray, $method );
        }
        // $responseData = $this->apiService->getData( $token, $apiUrl1, $body, $method );
        // dd($responseData);
    ...
    public function edit_product( Request $request ) {
        ...
        // dd($responseData);
        $product_categories = $responseData[ 'product_category' ];
        return view( 'front/mdhealth/vendor/vendor_add_products', compact( 'product_list','product_categories' ) );
    }
