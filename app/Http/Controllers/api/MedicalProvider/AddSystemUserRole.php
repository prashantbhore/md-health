...
class AddSystemUserRole extends BaseController
{
    ...
    public function add_system_user(Request $request)
    {
        ...
        $updateUniqueId = MedicalProviderRegistrater::where('id', $mdProviderRegistration->id)
            ->update(['provider_unique_no' => $providerUniqueId]);
    ...
    public function provider_system_user_list()
    {
        ...
        if (!empty($sytem_user_list)) {
            ...
            foreach ($sytem_user_list as $system_user) {
                ...
                $selected_data[] = [
                    'id' => $system_user->id,
                    'provider_unique_no' => !empty($system_user->provider_unique_no) ? $system_user->provider_unique_no : '',
                    'company_name' => !empty($system_user->name) ? $system_user->company_name : '',
                    'email' => !empty($system_user->email) ? $system_user->email : '',
                    'role_name' => !empty($system_user->role->role_name) ? $system_user->role->role_name : '',
                ];
            }
    ...
    public function edit_system_user(Request $request)
    {
        ...
        if (!is_null($sytem_user_list)) {
            $selected_data = [
                ...
                'provider_unique_no' => $sytem_user_list->provider_unique_no,
