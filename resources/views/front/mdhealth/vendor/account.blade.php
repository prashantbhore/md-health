            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="form-div">
                                <form action="{{ url('md-update-vendor-profile') }}" method="post"
                                    enctype="multipart/form-data" id="accountmedpro">
                                    @csrf
                                    <div class="form-group mb-3">
                                        <label class="form-label">Company Name</label>
                                        <input type="text" name="company_name"
                                            value="{{ $medical_provider_list->company_name }}" class="form-control"
                                            id="company_name" aria-describedby="foodname"
                                            placeholder="Memorial Hospitals Ltd. Sti.">
                                    </div>
                                        ...
                                        <div class="form-group d-flex flex-column w-50 pe-2">
                                            ...
                                            <select name="country_id" id="country_id" class="form-select">
                                                @foreach ($countries as $country)
                                                    <option
                                                        value="{{ $country->id }}"{{ $country->id == $medical_provider_list->country_id ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group d-flex flex-column w-50 ps-2">
                                            ...
                                            <select name="city_id" id="city_id" class="form-select">
                                                @foreach ($cities as $city)
                                                    <option
                                                        value="{{ $city->id }}"{{ $city->id == $medical_provider_list->city_id ? 'selected' : '' }}>
                                                        {{ $city->city_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" name="tax_no" value="{{ $medical_provider_list->tax_no }}"
                                            class="form-control" id="tax_no" aria-describedby="foodname"
                                            placeholder="TAX Number">
                                    </div>

                                    <div class="form-group mb-3">
                                        <label class="form-label">Authorized Person Full Name</label>
                                        <input type="text" name="authorisation_full_name"
                                            value="{{ $medical_provider_list->authorisation_full_name }}"
                                            class="form-control" id="authorisation_full_name" aria-describedby="foodname"
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <textarea class="form-control" name="company_overview" value="" id="company_overview" rows="4"
                                        ...
                                        <div class="preview-img gallery">
                                            ...
                                            @if (!empty($ProviderImagesVideos))
                                                @foreach ($ProviderImagesVideos as $ProviderImagesVideo)
                                                    @php
                                                        $fileExtension = pathinfo($ProviderImagesVideo->vendor_product_image_path, PATHINFO_EXTENSION);
                                                    @endphp
                                                    ...
                                                    @if ($fileExtension === 'mp4')
                                                        <div class="prev-img-div video-card"
                                                            ...
                                                            <video class="video-div" controls>
                                                                <source
                                                                    src="{{ !empty($ProviderImagesVideo->vendor_product_image_path) &&
                                                                    Storage::exists($ProviderImagesVideo->vendor_product_image_path)
                                                                        ? url('/') . Storage::url($ProviderImagesVideo->vendor_product_image_path)
                                                    ...
                                                    @else
                                                        <div class="prev-img-div"
                                                            id="img_div_{{ $ProviderImagesVideo->id }}">
                                                            <a href="{{ !empty($ProviderImagesVideo->vendor_product_image_path) &&
                                                            Storage::exists($ProviderImagesVideo->vendor_product_image_path)
                                                                ? url('/') . Storage::url($ProviderImagesVideo->vendor_product_image_path)
                                                                : URL::asset('admin_panel/commonarea/dist/img/default.png') }}"
                                                                class="glightbox">
                                                                <img src="{{ !empty($ProviderImagesVideo->vendor_product_image_path) &&
                                                                Storage::exists($ProviderImagesVideo->vendor_product_image_path)
                                                                    ? url('/') . Storage::url($ProviderImagesVideo->vendor_product_image_path)
                                                                    : URL::asset('admin_panel/commonarea/dist/img/default.png') }}"
                                                                    alt="{{ !empty($ProviderImagesVideo->vendor_product_image_name) ? $ProviderImagesVideo->vendor_product_image_name : '' }}" />
                                                            </a>
        ...
        function deleteClientLogo(client_logo_id) {
            ...
            if (client_logo_id != "") {
                if (confirm("Do you really want to delete this image ?")) {
                    $.ajax({
                        url: base_url + "/md-delete-vendor-images-videos",
